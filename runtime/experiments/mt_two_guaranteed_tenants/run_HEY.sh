...
declare -ir DURATION_SEC=30

# declare -ar CONCURRENCY=(0 2 4 9 15 18 20 25 30 36 38 40 45 50 60 70 80)
declare -ar CONCURRENCY=(30 36 37 40 45 50 60 70 80) # for quick debuging
...
run_experiments() {
	...
	for con in "${CONCURRENCY[@]}"; do
		...
		hey -disable-compression -disable-keepalive -disable-redirects -z "$((DURATION_SEC+OFFSET))"s -n "$ITERATIONS" -c 18 -o csv -m GET -d "30\n" "http://$hostname:10030" > "$results_directory/fib30_g1_con$con.csv" 2> /dev/null &
		fib30_g1_PID="$!"

		# sleep "$OFFSET"s
		
		if ((con > 0)); then
			hey -disable-compression -disable-keepalive -disable-redirects -z "$DURATION_SEC"s -n "$ITERATIONS" -c "$con" -o csv -m GET -d "30\n" "http://$hostname:20030" > "$results_directory/fib30_g2_con$con.csv" 2> /dev/null &
			fib30_g2_PID="$!"
			...
			get_result_count "$results_directory/fib30_g2_con$con.csv" || {
				printf "\tfib30_g2_con%d: [ERR]\n" "$con"
				panic "fib30_g2_con$con has zero requests."
				return 1
			}
		else 
			touch "$results_directory/fib30_g2_con$con.csv"
		fi
		...
		get_result_count "$results_directory/fib30_g1_con$con.csv" || {
			printf "\tfib30_g1: [ERR]\n"
