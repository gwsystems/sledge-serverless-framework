...
current_sandbox_set(struct sandbox *sandbox)
{
	...
	if (sandbox == NULL) {
		...
		current_sandboxes[global_worker_thread_idx]               = NULL;
		runtime_worker_threads_deadline[global_worker_thread_idx] = UINT64_MAX;
	} else {
		...
		current_sandboxes[global_worker_thread_idx]               = sandbox;
		runtime_worker_threads_deadline[global_worker_thread_idx] = sandbox->absolute_deadline;
	}
}
