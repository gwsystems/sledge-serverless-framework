...
software_interrupt_handle_signals(int signal_type, siginfo_t *signal_info, void *user_context_raw)
{
	...
	case SIGALRM: {
		...
		/* NOOP if software if software interrupts not enabled */
		if (!software_interrupt_is_enabled()) return;

		/* FIXME: Why might context switches have software interrupts enabled */
	...
	default:
		panic("Handler unexpectedly called for masked signal\n");
	}
#endif
}
