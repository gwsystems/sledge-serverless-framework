...
#define UV_FS_REQ_INIT() { .data = sandbox_current(), .result = 0 }
...
wasm_open(i32 path_off, i32 flags, i32 mode)
{
	...
	debuglog("[%p] end[%d]\n", uv_fs_get_data(&req), ret);
	uv_fs_req_cleanup(&req);
	if (ret < 0) io_handle_close(iofd);
	else         io_handle_preopen_set(iofd, ret);
...
wasm_readv(i32 fd, i32 iov_offset, i32 iovcnt)
{
	...
	for (int i = 0; i < iovcnt; i += RDWR_VEC_MAX) {
		...
		for (j = 0; j < RDWR_VEC_MAX && i + j < iovcnt; j++) {
			bufs[j] = uv_buf_init(get_memory_ptr_void(iov[i + j].base_offset, iov[i + j].len), iov[i + j].len);
		}
...
wasm_connect(i32 sockfd, i32 sockaddr_offset, i32 addrlen)
{
	...
	if (t == UV_TCP) {
		...
		debuglog("[%p] connect\n", c);
		int r = uv_tcp_connect(&req, (uv_tcp_t *)h, get_memory_ptr_void(sockaddr_offset, addrlen), wasm_connect_callback);
		sandbox_block();
