...
# declare -ir ITERATIONS=10000 # ignored when DURATION_SEC is used
declare -ir DURATION_SEC=15
declare -ir NWORKERS=18

# declare -ar CONCURRENCY=(0 2 4 9 15 18 20 25 30 36 38 40 45 50 60 70 80)
declare -ar CONCURRENCY=(70 80) # for quick debuging
...
declare -ar WORKLOADS=(fib30_g1 fib30_g2)
...
run_experiments() {
	...
	local -ir g1_con=$NWORKERS
	local -ir g1_deadline_us=${DEADLINES_US[${WORKLOADS[0]}]}
	local -ir g2_deadline_us=${DEADLINES_US[${WORKLOADS[1]}]}

	local -ir g1_rps=$((1000000*g1_con/g1_deadline_us))
	...
	for con in "${CONCURRENCY[@]}"; do
		...
		g2_rps=$((1000000*con/g2_deadline_us))

		loadtest -t "$((DURATION_SEC+OFFSET))"  -c "$g1_con" --rps "$g1_rps" -P "30" "http://$hostname:10030" > "$results_directory/fib30_g1_con$con.txt" 2> /dev/null &
		fib30_g1_PID="$!"

		# sleep "$OFFSET"s
		
		if ((con > 0)); then
			loadtest -t "$((DURATION_SEC+OFFSET))"  -c "$con" --rps "$g2_rps" -P "30" "http://$hostname:20030" > "$results_directory/fib30_g2_con$con.txt" 2> /dev/null &
			fib30_g2_PID="$!"

			wait -f "$fib30_g2_PID" || {
				printf "\tfib30_g2_con%d: [ERR]\n" "$con"
				panic "failed to wait -f ${fib30_g2_PID}"
				return 1
			}
			get_result_count "$results_directory/fib30_g2_con$con.txt" || {
				printf "\tfib30_g1_con%d: [ERR]\n" "$con"
				panic "fib30_g2_con$con has zero requests."
				return 1
			}
		else 
			touch "$results_directory/fib30_g2_con$con.txt"
		fi

		wait -f "$fib30_g1_PID" || {
			printf "\tfib30_g1: [ERR]\n"
			panic "failed to wait -f ${fib30_g1_PID}"
			return 1
		}
		get_result_count "$results_directory/fib30_g1_con$con.txt" || {
			printf "\tfib30_g1: [ERR]\n"
			panic "fib30_g1 has zero requests."
			return 1
		}
...
process_client_results() {
	...
	for workload in "${WORKLOADS[@]}"; do
		...
		printf "Concurrency,%s_SR\n" "$workload" >> "$results_directory/success.csv"
		printf "Concurrency,Throughput\n" >> "$results_directory/throughput.csv"
		percentiles_table_header "$results_directory/latency.csv" "Con"

		for con in "${CONCURRENCY[@]}"; do

			if [[ ! -f "$results_directory_root/${workload}_con$con.txt" ]]; then
				printf "[ERR]\n"
				error_msg "Missing $results_directory_root/${workload}_con$con.txt"
				return 1
			fi
			...
			total=$(grep "Completed requests:" "$results_directory_root/${workload}_con$con.txt" | tr -s ' ' | cut -d ' ' -f 13)
			# missed=$(grep "Total errors:" "$results_directory_root/${workload}_con$con.txt" | tr -s ' ' | cut -d ' ' -f 13)
			missed=$((total/2))
			ok=$((total-missed))
			...
			throughput=$(grep "Requests per second" "$results_directory_root/${workload}_con$con.txt" | tr -s ' '  | cut -d ' ' -f 14 | tail -n 1)
			printf "%d,%d\n" "$con" "$throughput" >> "$results_directory/throughput.csv"

			# Generate Latency Data
			min=0
			p50=$(echo "$(grep 50% "$results_directory_root/${workload}_con$con.txt" | tr -s ' ' | cut -d ' ' -f 12)*1000" |bc)
			p90=$(echo "$(grep 90% "$results_directory_root/${workload}_con$con.txt" | tr -s ' ' | cut -d ' ' -f 12)*1000" |bc)
			p99=$(echo "$(grep 99% "$results_directory_root/${workload}_con$con.txt" | tr -s ' ' | cut -d ' ' -f 12)*1000" |bc)
			p100=$(echo "$(grep 100% "$results_directory_root/${workload}_con$con.txt" | tr -s ' ' | cut -d ' ' -f 12 | tail -n 1)*1000" |bc)
			mean=$(echo "scale=1;$(grep "Mean latency:" "$results_directory_root/${workload}_con$con.txt" | tr -s ' ' | cut -d ' ' -f 13)*1000" |bc)

			printf "%d,%d,%d,%.1f,%d,%d,%d,%d\n" "$con" "$total" "$min" "$mean" "$p50" "$p90" "$p99" "$p100" >> "$results_directory/latency.csv"
